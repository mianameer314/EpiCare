...
import { BrainCircuit, Upload, History, Filter, Search, PlusCircle, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="eeg-page">
          ...
          {sessions.length > 0 && (
            <div style={{ marginTop: 'var(--space-8)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-4)' }}>
                ...
                <button
                  onClick={() => setActiveTab('history')}
                  style={{ background: 'none', border: 'none', color: 'var(--color-secondary)', fontSize: 'var(--text-sm)', cursor: 'pointer', fontWeight: 500 }}
                >
      ...
      {activeTab === 'history' && (
        ...
        >
          {/* ── Filter Bar ── */}
          <div className="eeg-filter-bar glass-card" style={{ padding: 'var(--space-3) var(--space-4)', marginBottom: 'var(--space-6)', display: 'flex', gap: 'var(--space-3)', alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flex: 1, minWidth: '200px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '0 var(--space-3)' }}>
          ...
          {isLoading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 'var(--space-4)' }}>
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="glass-card" style={{ padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  <div className="skeleton skeleton-text" style={{ width: '60%' }} />
                  <div className="skeleton skeleton-text" style={{ width: '40%', height: '0.8em' }} />
                  <div className="skeleton" style={{ height: '32px', borderRadius: 'var(--radius-md)', marginTop: 'var(--space-2)' }} />
                </div>
              ))}
            </div>
          ) : filteredSessions.length === 0 ? (
            <div className="glass-card" style={{ padding: 'var(--space-12)', textAlign: 'center' }}>
              <BrainCircuit size={48} style={{ color: 'var(--color-text-placeholder)', margin: '0 auto var(--space-4)', opacity: 0.4 }} />
              ...
              <p style={{ color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)', marginBottom: 'var(--space-6)' }}>
                {searchQuery || statusFilter ? 'No recordings match your search filter.' : 'You have not uploaded any EEG recordings yet.'}
              </p>
              <button
                onClick={() => setActiveTab('upload')}
