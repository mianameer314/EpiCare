...
export function RecommendationCard({ recommendation, onDismiss }: RecommendationCardProps) {
  ...
  const { data: whyData, isLoading: whyLoading } = useQuery({
    ...
    queryFn: () => recommendationsApi.getWhyShown(recommendation.id),
    enabled: showWhy,
  });
  ...
  return (
    <div className={`recommendation-card ${isVictoryLap ? 'victory-lap' : ''}`}>
      ...
      {showWhy && (
        <div className="recommendation-why">
          {whyData?.condition_description || recommendation.rationale ? (
            <>
              <strong>Clinical Rationale:</strong> {whyData?.condition_description || recommendation.rationale}
            </>
          ) : whyLoading ? (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <Loader2 size={12} className="animate-spin" /> Loading clinical rationale...
            </span>
          ) : (
            <>
              <strong>Clinical Rationale:</strong> Generated by clinical recommendation engine based on recent telemetry.
            </>
          )}
        </div>
      )}
