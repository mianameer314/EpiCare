...
from app.core.exceptions import error_response, not_found_error
...
async def analyze_session(
    ...
    # 2) Map channels -----------------------------------------------
    try:
        channel_indices, map_warnings = map_channels(
            validated.channel_labels, max_missing=5
        )
    except ValueError as exc:
        await _set_failed(db, session, str(exc))
        raise error_response(
            code="CHANNEL_MAPPING_FAILED",
            message=str(exc),
            status_code=400,
        ) from exc

    session.validation_result = _validation_dict(validated.validation, map_warnings)
    await db.commit()
    ...
    try:
        inference = predict(
            preprocessed.raw_windows,
            preprocessed.spectrogram_windows,
        )
    except Exception as exc:
        await _set_failed(db, session, str(exc))
        raise
